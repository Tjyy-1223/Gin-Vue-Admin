<template>
    <div class="card-view hidden animate-zoom-in animate-duration-600 text-center lg:block space-y-5">
        <div class="mt-4 flex justify-center">
            <img class="w-[105px] duration-600 hover:rotate-360" :src="convertImgUrl(blogConfig.website_avatar)"
                alt="author avatar">
        </div>
        <div class="space-y-1">
            <p class="text-2xl">
        ...
        <div class="flex justify-center text-center">
            <button
                class="h-9 w-[80%] flex items-center justify-center f-c-c transform rounded bg-blue-600 text-white transition-200 ease-in-out hover:scale-105 hover:bg-orange-600"
                @click="addToFavorites">
                <Icon icon="mdi-tag-multiple" style="font-size: 20px;" />
                <span class="mr-1 text-xl" /> 加入书签
            </button>
        </div>
...
function addToFavorites() {
    (window as any).$message?.info('按 CTRL + D 将本页加入书签')
}
</script>
